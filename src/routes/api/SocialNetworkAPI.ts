...
router.put('/users/:userId', async (_req: Request, res: Response) => {
  const updatedUser = await User.findByIdAndUpdate(_req.params.userId, _req.body, { new: true });
  res.json(updatedUser);
});

// DELETE to remove a user by its _id
router.delete('/users/:userId', async (_req: Request, res: Response) => {
...
router.post('/thoughts', async (_req: Request, res: Response) => {
...
router.get('/thoughts/:thoughtId', async (_req: Request, res: Response) => {
  const thought = await Thought.findById(_req.params.thoughtId);
  res.json(thought);
});

// DELETE to remove a thought by its _id
router.delete('/thoughts/:thoughtId', async (_req: Request, res: Response) => {
...
router.post('/users/:userId/friends/:friendId', async (_req: Request, res: Response) => {
  const user = await User.findById(_req.params.userId);
...
router.delete('/thoughts/:thoughtId/reactions/:reactionId', async (_req: Request, res: Response) => {
  const thought = await Thought.findById(_req.params.thoughtId);
